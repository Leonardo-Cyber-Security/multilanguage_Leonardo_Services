...
### Leonardo Kubernetes Service Reference Architecture 
...
SubnetDescription**Control-plane subnet**Restricted subnet for accessing the Kubernetes API (managed by Leonardo Cloud)**Node subnet(s)**Hosts system and user node pools**Ingress subnet**Load-balanced entrypoints (public or private)**Private services subnet**Internal services such as databases, caches, message brokers
